...
datCon=subset(dat, Cultivar %in% "Cabernet_Sauvignon")
datCon$Dataset=factor(datCon$Dataset)

levels(datCon$Dataset) #6 levels = 4 calibration, 2 validation

set.seed(0706) #June 7th 2022
calibration=factor(sample(levels(factor(datCon$Dataset)),4, replace=F))

calibration

calibration=factor(c("2016-2017", "2017-2018", "2018-2019", "2020-2021"))

validation=factor(c("2015-2016","2019-2020"))
...
kdeaccf =  data.frame(seq(1.8,1.3,-0.1)) 
  ...
  for (i in c(modelstart:nrow(wdat4))) {
    ...
    wdat4$CH[i]=wdat4$CH[i-1]+ 
      ((wdat4$DDcendo[i]*kaendo+wdat4$DDceco[i]*kaeco)*(1-((Hcmin - wdat4$CH[i-1])/(Hcmin - Hcmax)))+
         (wdat4$DDhendo[i]*kdendo)*((1- ((wdat4$CH[i-1]-Hcmax)/(Hcmin - Hcmax))^Theta))+
         (wdat4$DDheco[i]*kdeco)*  ((1- ((wdat4$CH[i-1]-Hcmax)/(Hcmin - Hcmax))^Theta)))
...
ggplot()+
  ...
  geom_abline(slope = 0,intercept=-24)+
  geom_segment(data=datConvalmeanb,aes(x=Date,xend=Date,y=CH,yend=LTE, col=(CH-LTE)))+
  geom_line(data=wdatvalb,aes(x=Date,y=CH))+
  geom_point(data=datConvalmeanb, aes(Date,LTE),shape=21, fill="white")+
  #geom_point(data=datConval, aes(Date,LTE, fill=PI),shape=21, stat="summary")+
